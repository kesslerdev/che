...
import static org.eclipse.che.workspace.infrastructure.openshift.provision.AsyncStorageProvisioner.ASYNC_STORAGE_CLAIM;
...
public class AsyncStorageProvisionerTest {
  ...
  @Test
  public void shouldNotCreatePvc()
    ...
    meta.setName(ASYNC_STORAGE_CLAIM);
    PersistentVolumeClaim pvc = new PersistentVolumeClaim();
    pvc.setMetadata(meta);
    when(pvcs.getItems()).thenReturn(singletonList(pvc));
    ...
    verify(osClient.configMaps().inNamespace(anyString()), times(1)).create(any(ConfigMap.class));
    verify(osClient.pods().inNamespace(anyString()), times(1)).create(any(Pod.class));
    verify(osClient.persistentVolumeClaims().inNamespace(anyString()), never())
        .create(any(PersistentVolumeClaim.class));
  }
